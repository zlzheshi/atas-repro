...
wait_for_two_gpus() {
  while true; do
    ...
    echo "[$(date '+%F %T')] waiting for 2 GPUs with >= ${MIN_FREE_MB} MiB free memory..." >&2
    nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits >&2
    sleep "$POLL_SECONDS"
  done
}

GPU_LIST=$(wait_for_two_gpus)
echo "[$(date '+%F %T')] starting author-aligned QuickGELU 2-GPU accum2 ImageNet run on GPUs ${GPU_LIST}"
CUDA_VISIBLE_DEVICES="$GPU_LIST" PYTHONPATH=. \
python -m torch.distributed.run \
  --standalone \
  --nproc_per_node=2 \
  train_atas.py \
  --config configs/atas_vitb_imagenet_full_author_quickgelu_2gpu_accum2.yaml \
  --data-root "$DATA_ROOT"
