...
CONFIG=${CONFIG:-configs/atas_vitb_subset_100x200_stable.yaml}
CHECKPOINT_DIR=${CHECKPOINT_DIR:-outputs/atas_vitb_subset_100x200_stable}
OUTPUT_ROOT=${OUTPUT_ROOT:-outputs/voc_maskclip_subset_100x200}
GPU=${GPU:-3}
...
CUDA_VISIBLE_DEVICES="$GPU" PYTHONPATH=. python scripts/evaluate_voc_zero_shot_seg.py \
  --config "$CONFIG" \
  --voc-root "$VOC_ROOT" \
  --model-name openclip_baseline_maskclip \
...
for epoch in $EPOCHS; do
  ...
  CUDA_VISIBLE_DEVICES="$GPU" PYTHONPATH=. python scripts/evaluate_voc_zero_shot_seg.py \
    ...
    --model-name "atas_epoch${epoch}_maskclip" \
